...
draft: false
featured: false
cover: "/images/covers/pinia-state-management-design.svg"
---
...
## SSR Hydration 与多 Tab

```ts
// plugins/pinia-persist.client.ts — 仅客户端 hydrate
export default defineNuxtPlugin(() => {
  const store = useUserStore();
  const cached = sessionStorage.getItem('user');
  if (cached) store.$patch(JSON.parse(cached));
  store.$subscribe(() => {
    sessionStorage.setItem('user', JSON.stringify(store.$state));
  });
});
```

**SSR 注意**：persist 插件标记 `.client.ts`，避免服务端读 `sessionStorage` 报错。多 Tab 同步用 `storage` 事件或 `BroadcastChannel` 广播 `$patch`。

## 迁移量化

Vuex → Pinia 迁移 14 个 module，**boilerplate -45%**，DevTools 调试步骤从 mutation+action 简化为直接追踪 action。
